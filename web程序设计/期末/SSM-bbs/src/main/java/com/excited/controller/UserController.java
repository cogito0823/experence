...
public class UserController {
    ...
    public String userLogin(User loginUser, HttpServletRequest request, RedirectAttributes redirect) {
        ...
        // 判断登录信息是否正确
        if (user.getUserState()==0 && user != null && loginUser.getPassword().equals(password)) {
    ...
    public String doFollow(String userName ,RedirectAttributes redirectAttributes){
        User user = userService.getUserByUserName(userName);
        user.setUserFollowCount(user.getUserFollowCount()+1);
        userService.updateUserByUserName(user);
