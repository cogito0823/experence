...
public class DES {
    ...
    int[] LFT={1,1,2,2,2,2,2,2,1,2,2,2,2,2,2,1};
    ...
    public static void main(String[] args){
        String message="hahjksakh";
        System.out.println("原文：\n"+message);
        DES dES=new DES("12345678",message);
        byte[] c=dES.docode(message.getBytes(),1);
        System.out.println("密文：\n"+new BigInteger(1,c).toString(2));
        byte[]p=dES.docode(c,0);
        System.out.println("明文：\n"+new String(p));
    }


    public byte[] docode(byte[] p ,int flag){           //加密信息并返回密文
        int length;
        length=p.length;
        int g_num;
        int r_num;
        g_num=length/8;                                 //明文组数
        r_num=8-(length-g_num*8);                       //计算mod8便于填充明文
        byte[] p_padding;                               //填充后的明文字节数组
        if (r_num<8){                                   //填充明文
            p_padding=new byte[length+r_num];
    ...
    public byte[] decode(byte[] p,int k[][],int flag){
        ...
        if (flag == 1) {                                    //加密轮置换输出为p_IP
            for (int i = 0; i < 16; i++) {
                lunZhiHuan(p_IP, i, flag, k[i]);
            }
        } else if (flag == 0) {                             //解密轮置换输出为p_IP
            for (int i = 15; i > -1; i--) {
                lunZhiHuan(p_IP, i, flag, k[i]);
            }
        }
        int[] c=new int[64];
        for(int i=0;i<IP_1.length;i++){                     //逆置换输出为数组c
            c[i]=p_IP[IP_1[i]-1];
        }
    ...
    public void lunZhiHuan(int[] M, int times, int flag, int[] sunkeyi){    //轮置换方法
        int[] L0=new int[32];
        int[] R0=new int[32];
        int[] L1=new int[32];
        int[] R1=new int[32];
